...
function runGame(){
    ...
    if (firstrun = false) {
        frogDivs[0].removeEventListener("click", handler);
        frogDivs[1].removeEventListener("click", handler);
        frogDivs[2].removeEventListener("click", handler);
        frogDivs[3].removeEventListener("click", handler);
        frogDivs[4].removeEventListener("click", handler);
    }
    ...
    for (let frog of frogDivs) {
        frog.addEventListener("click", function handler(e) {
            ...
            if (frogDivs[frogSeq[j]] === e.target.parentElement) {
                ...
                document.getElementById("frog0").removeEventListener("click", handler);
                firstrun = false
                frogDivs[0].removeEventListener("click", handler);
                frogDivs[1].removeEventListener("click", handler);
                frogDivs[2].removeEventListener("click", handler);
                ...
                for (let k = 0; k < frogDivs.length; k++) {
                    frogDivs[k].removeEventListener("click", handler);
                    console.log(frogDivs[k]);
                }
                toggleWelcome(welcome);
            }
        }) 
